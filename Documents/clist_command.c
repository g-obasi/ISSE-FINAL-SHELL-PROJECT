...
{
    CList_command list = (CList_command) malloc(sizeof(struct _clist_command));
    assert(list);
...
//Documented in .h file
void CL_push(CList_command list, CListElementType element)
{
    assert(list);
...
// Documented in .h file
void CL_append(CList_command list, CListElementType element) {
...
CListElementType CL_nth(CList_command list, int pos) {
...
// Documented in .h file
CListElementType CL_remove(CList_command list, int pos) {
    assert(list);
...
bool CL_insert(CList_command list, CListElementType element, int pos)
{
    assert(list);
...
// Documented in .h file
CList_command CL_copy(CList_command list)
{
    assert(list);
...
void CL_join(CList_command list1, CList_command list2) {
    ...
    while (list2->length > 0) {
        CL_append(list1, Command_pop(list2));
    }

}


void CL_reverse(CList_command list) {
    assert(list);
...
// Documented in .h file
void CL_foreach(CList_command list, CL_foreach_callback callback, void *cb_data) {
    assert(list);
...
bool CL_is_empty(CList_command list) {
